...
int  Get_pesq_mos(char* pchRefFileName, char *pchDegFileName, long lSampleRate, float& fPesqMosRet)
{
    SIGNAL_INFO ref_info;
    SIGNAL_INFO deg_info;
    ERROR_INFO err_info;
    if(pchDegFileName == 0 || pchRefFileName == 0)
        return 0;
    if(strcmp(pchDegFileName, "") == 0 || strcmp(pchRefFileName, "") == 0)
        return 0;
    long Error_Flag = 0;
    ...
    CM_INFO_TRACE("pesq::copy name = "<<pchRefFileName<<", degfile = "<<pchDegFileName);
    strcpy (ref_info.path_name, pchRefFileName); 
    strcpy (deg_info.path_name, pchDegFileName);
    ...
    CM_INFO_TRACE("pesq::select_rate sample rate = "<<lSampleRate);
    select_rate (lSampleRate, &Error_Flag, &Error_Type);
    CM_INFO_TRACE("pesq::pesq_measure ref file = "<<ref_info.file_name<<", degfile = "<<deg_info.file_name);
    pesq_measure (&ref_info, &deg_info, &err_info, &Error_Flag, &Error_Type);
    
    CM_INFO_TRACE("pesq::pesq_measure mos = "<<err_info.pesq_mos<<", mapped mos = "<<err_info.mapped_mos);
    fPesqMosRet = 0.0;
    if (Error_Flag == 0) {
        if ( err_info.mode == NB_MODE )
            fPesqMosRet = err_info.pesq_mos;
        else
            fPesqMosRet = err_info.mapped_mos;
        return 0;
    } else {
        CM_INFO_TRACE("pesq::An error of type "<< Error_Flag);
        //printf ("An error of type %d ", Error_Flag);
        if (Error_Type != NULL) {
            //printf (" (%s) occurred during processing.\n", Error_Type);
            CM_INFO_TRACE("pesq "<< Error_Type<<" occurred during processing");
        } else {
    ...
    ERROR_INFO * err_info, long * Error_Flag, char ** Error_Type)
{
	...
	fStart = clock()/(float)CLOCKS_PER_SEC;
    ...
    if ((*Error_Flag) == 0)
    {
       CM_INFO_TRACE("pesq:Reading reference file "<<ref_info-> path_name);
    ...
    if ((*Error_Flag) == 0)
    {
       CM_INFO_TRACE("pesq:Reading degraded file = "<<deg_info-> path_name);
    ...
    {   
        ...
        if( ref_info->input_filter == 1 ) {
            CM_INFO_TRACE (" pesq:apply_filter ref");
        ...
        if( deg_info->input_filter == 1 ) {
            CM_INFO_TRACE (" pesq:apply_filter deg");
        ...
        CM_INFO_TRACE(" input_filter begin");
        input_filter( ref_info, deg_info, ftmp );

        CM_INFO_TRACE(" Variable delay compensation");
        calc_VAD (ref_info);
        calc_VAD (deg_info);
        ...
        CM_INFO_TRACE (" pesq:Acoustic model processing");
        ...
        CM_INFO_TRACE("pesq:mesure fopen result file"<<ITU_RESULTS_FILE);
        if (resultsFile != NULL) {
            long start, end;
            ...
            fDiff = clock()/(float)CLOCKS_PER_SEC - fStart;
            fprintf (resultsFile, "It takes  %.3f seconds\t ", fDiff);
    ...
    CM_INFO_TRACE("pesq:mesure return");
