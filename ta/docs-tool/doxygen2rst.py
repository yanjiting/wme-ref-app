...
class DoxyGen2RST(object):
    ...
    def build_references(self):
        for file in _glob(self.rst_dir, *self.filter):
            ...
            for m in it:
                #print(m.groups())
                names = m.groups()[1].split('#')
                kind = m.groups()[0]
                if(kind == "define"):
                    kind = "file"
                ref_id = self._find_ref_id(kind, names[0])
                if(ref_id is None):
                    print("Reference is NOT found for: %s=%s" % (kind, m.groups()[1]))
                    continue
    ...
    def _build(self, m):
        ...
        if(m.groups()[0] == "uml"):
            ...
            linkTargets = m.groups()[1].split('#')
            if(len(linkTargets) > 1):
                link = linkTargets[0] + self.page_ext + '#' + linkTargets[1].lower()
            else:
                link = m.groups()[1] + self.page_ext
            retstr = ("`%s <%s>`_" % (m.groups()[2], link))
        elif(m.groups()[0] == "include"):
            retstr += ".. code:: python\n\n";
            extFilePath = os.path.join(self.rst_dir, m.groups()[1])
            with open(extFilePath) as f:
                content = f.readlines()
                f.close()
                for line in content:
                    retstr += "    " + line;
        else:
            if(m.groups()[0] != "function" and m.groups()[0] != "define"):
    ...
    def _build_ref(self, node):
        ...
        if target in self.page_references:
            ...
            self.page_references[target] = (node.attrib["refid"], node.attrib["kindref"], tokens[-1])
        ...
        for child in para:
            ...
            if(child.tag == "simplesect"):
                for child_para in child:
                    returntypestr = "" #"| "
                    if(child.attrib["kind"] == "return" or child.attrib["kind"] == "see"):
                        return_str = self._build_para(child_para)
                        retstr += "    " + returntypestr + child.attrib["kind"].upper() + ":" + LINE_BREAKER
                        for line in return_str.splitlines():
                            retstr += "".ljust(4, " ") + returntypestr + line + LINE_BREAKER
                    elif(child_para.tag == "title" and child_para.text):
                        lf.make_para_title(child_para.text.strip(), 4)
                    elif(child_para.tag == "para"):
                        retstr += self._build_para(child_para)
                    elif(child_para.text):
                        retstr += "".ljust(4, " ") + returntypestr + child_para.text.strip() + LINE_BREAKER
            if(child.tag == "preformatted"):
                retstr += "::" + LINE_BREAKER + LINE_BREAKER
                if(child.text):
                    for line in child.text.splitlines():
                        retstr += "  " + line + LINE_BREAKER
            if(child.tag == "ulink" and child.text):
                retstr += '`' + child.text.strip() + " <" + child.attrib["url"] + ">" + '`_'
            if(child.tag == "ref" and child.text):
    ...
    def _build_class(self, node):
        ...
        desc_node = self.get_desc_node(node)
    ...
    def build_function(self, member, sep = '*'):
        retstr = ""
        ...
        retstr += self._build_title(member.findtext("name"), sep, ref = ref_id)
        retstr += self.get_text(desc_node)
        retstr += LINE_BREAKER
        detail_node = member.find("detaileddescription")
    ...
    def _build_missed_types_and_structs(self):
        ...
        while (len(self.missed_types_structs) > 0):
            for key, value in self.missed_types_structs.iteritems():
                ...
                #print(value)
            not_found_structs.update(self.missed_types_structs)
            self.missed_types_structs = {}
            footnote += self._build_page_ref_notes()
            new_found_structs = {}
            for key in self.missed_types_structs:
                if(not key in not_found_structs):
                    new_found_structs[key] = self.missed_types_structs[key]
            self.missed_types_structs = new_found_structs
            
        fout.write(footnote)
        print("Not found elements:<<<<<<<<<<<<<<<<<<\r\n" + str(not_found_structs))
        print(">>>>>>>>>>>>>>>>>")
    ...
    def _build_page_ref_notes(self):
        ...
        for key, value in self.page_references.iteritems():
            ...
            if(value[0] in self.references):
                rstname = self.references[value[0]][2]
            elif page in self.references:
                rstname = self.references[page][2]
                if(value[0] != page):
                    anchor = anchor.split("::")[-1]
                    #print("<<<<<<<<<<%s==%s" % (anchor, self.references[page][4]))
                else:    
    ...
    def _build_defines(self, compound, name):
        rows = []
        rows.append(("Define", "Value", "Descritpion"))
        for member in compound.iter("memberdef"):
            if(member.attrib["kind"] == "define"):
                defName = member.findtext("name")
                defValue = member.findtext("initializer")
                defDesc = self.get_text(self.get_desc_node(member))
                rows.append((defName, defValue, defDesc))
    ...
    def _build_section_func(self, compound, ref_id, name):
        retstr = ""
        for member in compound.iter("memberdef"):
            if(member.attrib["kind"] == "function" and member.attrib["id"] == ref_id):
                retstr += self.build_function(member, '-')
    ...
    def convert_doxy(self, type, name):
        ...
        #mac doxygen will not generate file_*.xml, so we need to extract them directly from namespace XML.
        retstr = ""
        if(compound_kind == "namespace" and type == "function"):
            for section in compound.iter("sectiondef"):
                if(section.attrib["kind"] == "func"):
                    retstr += self._build_section_func(section, ref_id, name)
                    break
            return retstr;
        
        if(type == "define"):
            for section in compound.iter("sectiondef"):
                if(section.attrib["kind"] == "define"):
                    retstr += self._build_defines(section, name)
                    break
            return retstr;
...
if __name__ == '__main__':
    ...
    parser.add_argument("-i", "--input", default="example", help="Input file path of doxygen output and source rst file.")
    parser.add_argument("-o", "--output", default="example/output", help="Output converted restructured text files to path.")
