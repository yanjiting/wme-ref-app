...
LogMessage::~LogMessage()
{
  ...
  std::string formmatedMsg = g_logger_instance->ComposeMsg(log_entry_);
  //output to debug output
  std::wstring ws;
  ws.assign(formmatedMsg.begin(), formmatedMsg.end());
  OutputDebugString(ws.c_str());

  saveToLogger(formmatedMsg); // message saved
}
...
void LogMessage::messageSave(const char *printf_like_message, ...)
{
  ...
  const int nbrcharacters = vsnprintf_s(finished_message, sizeof(finished_message), printf_like_message, arglist);
  va_end(arglist);
  if (nbrcharacters <= 0)
  {
