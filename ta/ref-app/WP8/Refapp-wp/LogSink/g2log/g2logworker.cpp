...
* ********************************************* */
/* 2015-04-29 Xiuquan Hao (Cisco Systems, Inc) */
...
std::string pathSanityFix(std::string path, std::string file_name) {
    ...
    std::string tmp = path.insert(path.size(), file_name);
    return path;
}
...
{
  std::string str;
  str.append(verified_prefix.c_str());
  //str.append(g2::localtime_formatted(g2::systemtime_now(), file_name_time_formatted));
  str.append(".log");
...
void g2LogWorkerImpl::backgroundFileWrite(LogEntry message)
{
  ...
  if (!out.is_open() && isStartLog_)
  {
	  bool success_with_open_file = openLogFile(log_file_with_path_, out);
	  if (false == success_with_open_file)
	  {
		  return;
	  }
...
std::string g2LogWorkerImpl::formatMessage(std::string msg)
{
	auto system_time = g2::systemtime_now();
	auto steady_time = std::chrono::steady_clock::now();
	std::tm gm_time;
	gmtime_s(&gm_time,&system_time);


	std::stringstream _formatted_str;
	_formatted_str << "\n" << std::put_time(&gm_time, date_formatted.c_str());  // format example: //"%Y/%m/%d %H:%M:%S");
	_formatted_str << " " << std::put_time(&gm_time, time_formatted.c_str());  // format example: //"%Y/%m/%d %H:%M:%S");
	_formatted_str << "."<<std::chrono::duration_cast<std::chrono::microseconds>(steady_time - steady_start_time_).count(); //microseconds TODO: ta in min g2clocka här StopWatch
	_formatted_str << "\t" << msg;

	return _formatted_str.str();
}

void g2LogWorkerImpl::closeFile()
{
	//backgroundFileWrite("Log flushed successfully to disk \nExiting");
	filestream().close();
}

void g2LogWorkerImpl::startLog(bool isStart)
{
	isStartLog_ = isStart;
	if (!isStartLog_)
		closeFile();
...
void g2LogWorker::close()
{
	pimpl_->closeFile();
}

std::string g2LogWorker::ComposeMsg(std::string msg)
{
	return pimpl_->formatMessage(msg);
}

void g2LogWorker::startLog(bool isStart)
{
	pimpl_->startLog(isStart);
}
