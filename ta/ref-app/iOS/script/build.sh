...
echo "Start Integration Tests Build for iOS"
if [ ${CODE_COVERAGE_CONFIG}x = "x" ]; then
exitOnFailure xcodebuild ARCHS="armv7 arm64" VALID_ARCHS="armv7 armv7s arm64" ONLY_ACTIVE_ARCH=NO ${PROJECT_CLEAN} build -project $WMETEST_PROJECT -target MediaSessionIntegrationTest -configuration Release -sdk iphoneos
else
exitOnFailure xcodebuild ARCHS="armv7 arm64" VALID_ARCHS="armv7 armv7s arm64" ONLY_ACTIVE_ARCH=NO ${PROJECT_CLEAN} build -xcconfig "${CODE_COVERAGE_CONFIG}" -project $WMETEST_PROJECT -target MediaSessionIntegrationTest -configuration Release -sdk iphoneos
fi
...
#if [ ${CODE_COVERAGE_CONFIG}x = "x" ]; then
#exitOnFailure xcodebuild ARCHS="armv7 armv7s arm64" VALID_ARCHS="armv7 armv7s arm64" ONLY_ACTIVE_ARCH=NO ${PROJECT_CLEAN} build -project $WMETEST_PROJECT -target MediaSessionIntegrationTest -configuration Release -sdk iphonesimulator
#else
#exitOnFailure xcodebuild ARCHS="armv7 armv7s arm64" VALID_ARCHS="armv7 armv7s arm64" ONLY_ACTIVE_ARCH=NO ${PROJECT_CLEAN} build -xcconfig "${CODE_COVERAGE_CONFIG}" -project $WMETEST_PROJECT -target MediaSessionIntegrationTest -configuration Release -sdk iphonesimulator
#fi
