...
int
DownConvert::upsamplingDyadic( unsigned char*    pucBufferYUV[3],   int iStrideY,int iStrideUV,
                               unsigned char*    pucDstBufferYUV[3],   int iDstStrideY,int iDstStrideUV,
                               ResizeParameters* pcParameters )
{
  ...
  {
    //===== luma =====
    xCopyToImageBuffer    ( pucBufferYUV[0], iWidth,      iHeight,      iStrideY );
    xCompUpsamplingDyadic (             iWidth,      iHeight,      false    );
    xCopyFromImageBuffer  ( pucDstBufferYUV[0], iWidth << 1, iHeight << 1, iDstStrideY );
  	//===== chroma cb =====
    xCopyToImageBuffer    ( pucBufferYUV[1], iWidth >> 1, iHeight >> 1, iStrideUV );
    xCompUpsamplingDyadic (             iWidth >> 1, iHeight >> 1, true     );
    xCopyFromImageBuffer  ( pucDstBufferYUV[1], iWidth,      iHeight,      iDstStrideUV );
    //===== chroma cb =====
    xCopyToImageBuffer    ( pucBufferYUV[2], iWidth >> 1, iHeight >> 1, iStrideUV );
    xCompUpsamplingDyadic (             iWidth >> 1, iHeight >> 1, true     );
    xCopyFromImageBuffer  ( pucDstBufferYUV[2], iWidth,      iHeight,      iDstStrideUV );
    iWidth  <<= 1;
    iHeight <<= 1;
  }
    return 0;
...
int
DownConvert::upsamplingDyadic( unsigned char*    pucBufferY,unsigned char*    pucBufferU,unsigned char*    pucBufferV,   int iStrideY,int iStrideUV,
                              unsigned char*    pucDstBufferY, unsigned char*    pucDstBufferU, unsigned char*    pucDstBufferV,   int iDstStrideY,int iDstStrideUV,
                              ResizeParameters* pcParameters )
{
    int iWidth  = pcParameters->m_iRefLayerFrmWidth;
    int iHeight = pcParameters->m_iRefLayerFrmHeight;
    ...
    for( int i = iStages; i > 0; i-- )
    {
        ...
        xCompUpsamplingDyadic (             iWidth,      iHeight,      false    );
        xCopyFromImageBuffer  ( pucDstBufferY, iWidth << 1, iHeight << 1, iDstStrideY );
        //===== chroma cb =====
        xCopyToImageBuffer    ( pucBufferU, iWidth >> 1, iHeight >> 1, iStrideUV );
        xCompUpsamplingDyadic (             iWidth >> 1, iHeight >> 1, true     );
        xCopyFromImageBuffer  ( pucDstBufferU, iWidth,      iHeight,      iDstStrideUV );
        //===== chroma cb =====
        xCopyToImageBuffer    ( pucBufferV, iWidth >> 1, iHeight >> 1, iStrideUV );
        xCompUpsamplingDyadic (             iWidth >> 1, iHeight >> 1, true     );
        xCopyFromImageBuffer  ( pucDstBufferV, iWidth,      iHeight,      iDstStrideUV );
        iWidth  <<= 1;
        iHeight <<= 1;
    }
    return 0;
...
void
DownConvert::upsampling6tapBilin(  unsigned char*    pucBufferY,unsigned char*    pucBufferU,unsigned char*    pucBufferV,   int iStrideY,int iStrideUV,
                                 unsigned char*    pucDstBufferY, unsigned char*    pucDstBufferU, unsigned char*    pucDstBufferV,   int iDstStrideY,int iDstStrideUV,
                                 ResizeParameters* pcParameters  )
{
  int iInWidth    = pcParameters->m_iRefLayerFrmWidth;
  ...
  xCopyFromImageBuffer    ( pucDstBufferY,   iGlobWidth, iGlobHeight, iDstStrideY );
  //===== parameters for chroma =====
  iInWidth    >>= 1;
  iInHeight   >>= 1;
  iGlobWidth  >>= 1;
  iGlobHeight >>= 1;
  //===== chroma cb =====
  xCopyToImageBuffer      ( pucBufferU,   iInWidth,   iInHeight,   iStrideUV );
  xCompUpsampling6tapBilin( pcParameters, true );
  xCopyFromImageBuffer    ( pucDstBufferU,   iGlobWidth, iGlobHeight, iDstStrideUV );
  //===== chroma cr =====
  xCopyToImageBuffer      ( pucBufferV,   iInWidth,   iInHeight,   iStrideUV );
  xCompUpsampling6tapBilin( pcParameters, true );
  xCopyFromImageBuffer    ( pucDstBufferV,   iGlobWidth, iGlobHeight, iDstStrideUV );
}
