...
begin
	ZipFile = Zip::File
	print "your ruby zip is > 1.0.0\n"
rescue
  begin
	ZipFile = Zip::ZipFile
	print "your ruby zip is < 1.0.0\n"
  rescue
    print "no supported ruby zip supported.\n"
  end  
end
...
def resign_test_server_hook(app)
  ...
  keystores = [JavaKeystore.keystore_from_settings, 
               JavaKeystore.read_keystore_with_default_password_and_alias("debug.keystore"),
               JavaKeystore.read_keystore_with_default_password_and_alias(File.join(ENV["HOME"], "/.android/debug.keystore"))
               ].compact
  if keystores.empty?
  ...
  Dir.mktmpdir do |workspace_dir|
    Dir.chdir(workspace_dir) do
      ...
      ZipFile.new("dummy.apk").extract("AndroidManifest.xml","customAndroidManifest.xml")
      ZipFile.open("TestServer.apk") do |zip_file|
