...
class VSCobertura(object):
    ...
    def addCoverage(self, xml_coverage, filefilter=None, excludes=None):
        ...
        dom_data = minidom.parse(xml_coverage)
        files = dom_data.getElementsByTagName("SourceFileNames")
        outFiles = []
        for fileIt in files:
            filename = self.getText(fileIt, "SourceFileName")
            notfound = True
            ...
            for exclude in excludes:
                if exclude in filename:
                    ...
                    break
            if(not notfound):
                fileid = self.getText(fileIt, "SourceFileID")
        ...
        for fileInfo in outFiles:
            ...
            methods = dom_data.getElementsByTagName("Method")
            for method in methods:
                if(file_id == self.getText(method, "SourceFileID")):
                    ...
                    lines = method.getElementsByTagName("Lines")
                    for line in lines:
                        coverage = int(self.getText(method, "Coverage"))
                        line_start = int(self.getText(method, "LnStart"))
                        line_end = int(self.getText(method, "LnEnd"))
