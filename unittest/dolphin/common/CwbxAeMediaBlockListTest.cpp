...
class CWbxAeMediaBlockListTest : public testing::Test
{
public:
	CWbxAeMediaBlockListTest()
	{
	}

	virtual ~CWbxAeMediaBlockListTest()
	{
	}
...
TEST_F(CWbxAeMediaBlockListTest, CleanList)
{
...
TEST_F(CWbxAeMediaBlockListTest, GetDataPtr)
{
	CWbxAeMediaBlock * p1 = new CWbxAeMediaBlock(WBX_MEMORY_BLOCK_SIZE);
	p1->AdvanceWritePtr(100);
	EXPECT_EQ(WBXAE_SUCCESS,m_pList->PushBack(p1));
...
TEST_F(CWbxAeMediaBlockListTest, GetDataLen)
{
	DWORD temp = m_pList->GetDataLen();
	EXPECT_EQ(0,temp);

}
TEST_F(CWbxAeMediaBlockListTest, PushBack)
{
	CWbxAeMediaBlock * p1 = new CWbxAeMediaBlock(WBX_MEMORY_BLOCK_SIZE);
	p1->AdvanceWritePtr(100);
	EXPECT_EQ(WBXAE_SUCCESS,m_pList->PushBack(p1));
...
TEST_F(CWbxAeMediaBlockListTest, AlignFirst)
{
	CWbxAeMediaBlock * p1 = new CWbxAeMediaBlock(WBX_MEMORY_BLOCK_SIZE);
	p1->AdvanceWritePtr(100);
