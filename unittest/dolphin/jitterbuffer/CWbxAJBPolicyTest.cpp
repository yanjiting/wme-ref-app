...
TEST_F(CWbxAJBPolicyTest, JitterPolicy)
{
	...
	for ( int i =0 ; i < 100; i ++)
	{
		if ((i + 1)% 5 == 0)
		{
			...
			EXPECT_EQ(WBXAE_SUCCESS, m_pAJBPolicy->JitterPolicy(3000,80, (i + 10 ) * 20, i + 1 , 160* (i + 1), playtime));
			EXPECT_EQ(WBXAE_SUCCESS, m_pAJBPolicy->JitterPolicy(3000,80, (i + 1 )* 20 + 10, i, 160* i, playtime));
			i++;
		}
		else
		{
			EXPECT_EQ(WBXAE_SUCCESS, m_pAJBPolicy->JitterPolicy(3000,80, i * 20, i, 160* i, playtime));
		}
	...
	EXPECT_NE(WBXAE_SUCCESS, m_pAJBPolicy->JitterPolicy(3000,0, 20, 1, 160, playtime));

	EXPECT_NE(WBXAE_SUCCESS, m_pAJBPolicy->JitterPolicy(3000,800, 20, 1, 160, playtime));
}
...
TEST_F(CWbxAJBPolicyTest, GetJitterInformation)
{
	...
	for ( int i =0 ; i < 100; i ++)
	{
		if ((i + 1)% 5 == 0)
		{
			EXPECT_EQ(WBXAE_SUCCESS, m_pAJBPolicy->JitterPolicy(3000,80, (i + 1) * 20, i + 1, 160*( i + 1) , playtime));
			EXPECT_EQ(WBXAE_SUCCESS, m_pAJBPolicy->JitterPolicy(3000,80, i * 20, i, 160* i, playtime));
			i++;
		}
		else
	...
	EXPECT_EQ(WBXAE_SUCCESS, m_pAJBPolicy->JitterPolicy(3000,80, 103 * 20,103, 160* 103, playtime));
