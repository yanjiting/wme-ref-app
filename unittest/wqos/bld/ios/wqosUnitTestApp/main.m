...
extern int wqos_unittest_main(const char* path);
...
{
#ifdef IOS_SIMULATOR
    const char* path="/tmp/wqos_UT_ios_sim.xml";
#else
    char xmlWritePath[1024] = "";
    unsigned long uPathLen = 1024;
    char path[1024] = "";
    GetDocumentPath(xmlWritePath, uPathLen);
    sprintf(path, "%s%s", xmlWritePath, "/wqos_UT_ios_dev.xml");
#endif
    wqos_unittest_main(path);
