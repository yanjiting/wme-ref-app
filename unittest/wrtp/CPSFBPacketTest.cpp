...
class CPSFBPacketTEST : public ::testing::Test
{
protected:
    virtual void SetUp()
    {
...
TEST_F(CPSFBPacketTEST, CPSFBPacket)
{
    CScopedTracer test_info;
    CPSFBPacket fbPacketEncode(expect_ssrc, expect_ssrcSrc, (uint16_t)expect_fciLength*4, (uint8_t *)&*expect_fci.begin());
    ...
    //CPSFBPacket fbPacketDecode;
    //fbPacketDecode.Decode(os, mbEncode);

    EXPECT_EQ(RTCP_PT_PSFB, fb_packet->GetType());
